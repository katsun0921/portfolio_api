...
func (article *Article) FindByStatus(status string) ([]Article, rest_errors.RestErr) {
  stmt, err := articles_db.Client.Prepare(queryFindByStatus)
  if err != nil {
    logger.Error("error when trying to prepare find articles by status statement", err)
    return nil, rest_errors.NewInternalServerError("error when trying to find by status", errors.New("database error"))
  }
  ...
  for rows.Next() {
    var article Article
    if err := rows.Scan(&article.Id, &article.FirstName, &article.LastName, &article.Email, &article.DateCreated, &article.Status); err != nil {
      logger.Error("error when scan article row into article struct", err)
      return nil, rest_errors.NewInternalServerError("error when trying to find by status", errors.New("database error"))
    }
    results = append(results, article)
